...
class GerminalCenterLayout {
  ...
  determineZone(cellData) {
    ...
    if (cellData.type === "stem") {
      return "memoryEliteZone"; // Stem cells go to memory zone
    }
    ...
    if (cellData.type === "controller") {
      return "quantumDisk"; // Controllers in quantum layer
    }
  ...
  assignCellPosition(cellData, index, population) {
    ...
    switch (zone) {
      ...
      case "memoryEliteZone":
        ...
        position.copy(this.memoryEliteZonePosition(cellData, index));
        break;

      case "quantumDisk":
        // 4D projection arrangement for quantum cells
        position.copy(this.quantumDiskPosition(cellData, index));
        break;
...
class LiveNeuralClockwork {
  ...
  updatePopulationStructure(state) {
    ...
    const zoneCounts = {
      darkZone: 0,
      lightZone: 0,
      memoryEliteZone: 0, // CORRECTED
      quantumDisk: 0, // CORRECTED
      mantleZone: 0,
    };
    ...
    if (state.cells && state.cells.length > 0) {
      ...
      this.cells.forEach((cell, cellId) => {
        ...
        if (specialization && specialization.primary) {
          // This will now correctly increment 'biosensor', 'effector', etc.
  ...
  updateZoneCounts(zoneCounts) {
    ...
    const memoryEliteZoneEl = document.getElementById("memoryEliteZoneCount");
    const quantumDiskEl = document.getElementById("quantumDiskCount");
    ...
    if (memoryEliteZoneEl)
      memoryEliteZoneEl.textContent = `(${zoneCounts.memoryEliteZone})`; // CORRECTED
    if (quantumDiskEl)
      quantumDiskEl.textContent = `(${zoneCounts.quantumDisk})`; // CORRECTED
  ...
  determineCellSpecialization(cellData) {
    // The primary therapeutic type is now read directly from the data.
    let primary = cellData.type || "balanced";

    // This part remains useful for metrics.
    ...
    // --- CORRECTION 1: REMOVE THE INCORRECT MAPPING ---
    // The typeMapping logic is no longer needed. We will use the new types directly.
    // const typeMapping = { ... };
    ...
    // --- CORRECTION 3: RETURN THE CORRECT PRIMARY TYPE ---
    return {
      primary: primary, // Use the direct therapeutic type (e.g., 'biosensor')
      geneTypes,
      diversity,
      complexity,
      quantum_ratio: 0,
      active_ratio: 1,
      therapeuticGeneTypes,
    };
  }
