...
from scripts.depricated.transposable_immune_ai_production_complete import ContinuousDepthGeneModule
...
class TherapeuticSeedGene(ContinuousDepthGeneModule):
    """Base class for therapeutic seed genes - Maintains core transposition capabilities"""
    ...
    def __init__(self, gene_type: str, therapeutic_function: Dict):
        ...
        # Ensure we maintain core transposition attributes from parent
        ...
        self.biomarker_encoder = nn.Sequential(
            nn.Linear(cfg.num_biomarkers, 128),
            nn.ReLU(),
        ...
        self.safety_monitor = nn.Sequential(
            nn.Linear(cfg.hidden_dim + cfg.num_biomarkers, 64),
            nn.ReLU(),
            nn.Linear(64, 1),
...
class BiosensorGene(TherapeuticSeedGene):
    """Detects patient state and disease markers - Maps to V gene (Variable/Recognition)"""
    
    def __init__(self):
        super().__init__('V', {  # Changed from 'BS' to 'V' to match original architecture
            'function': 'biosensing',
            'targets': cfg.critical_biomarkers,
            'sensitivity': 'high',
            'response_time': 'fast'
        })
        
        # Map to original V gene position (0.15 ± 0.1)
...
class TherapeuticEffectorGene(TherapeuticSeedGene):
    """Produces therapeutic molecules/signals - Maps to D gene (Diversity/Action)"""
    
    def __init__(self, therapeutic_mode: str):
        super().__init__('D', {  # Changed from 'TE' to 'D' to match original architecture
        ...
        # Map to original D gene position (0.45 ± 0.1)
    ...
    def _calculate_safety(self, therapeutic: torch.Tensor, patient_state: Dict) -> torch.Tensor:
        ...
        device = next(self.parameters()).device
        biomarkers = patient_state.get('biomarkers', torch.zeros(cfg.num_biomarkers, device=device))
...
class AdaptiveControllerGene(TherapeuticSeedGene):
    """Controls and coordinates therapeutic response - Maps to J gene (Joining/Coordination)"""
    
    def __init__(self):
        super().__init__('J', {  # Changed from 'AC' to 'J' to match original architecture
        ...
        # Map to original J gene position (0.8 ± 0.1)
        ...
        self.treatment_planner = nn.LSTM(
            cfg.hidden_dim + cfg.num_biomarkers,
            cfg.hidden_dim,
            num_layers=3,
            batch_first=True
        )
        ...
        # Meta-learner input: treatment_features (hidden_dim) + response_features (hidden_dim) + objective_weights (4)
        meta_input_size = cfg.hidden_dim * 2 + 4
        self.meta_learner = nn.Sequential(
            nn.Linear(meta_input_size, cfg.hidden_dim),
            nn.ReLU(),
            nn.Linear(cfg.hidden_dim, cfg.hidden_dim // 2),
            nn.ReLU(),
    ...
    def plan_treatment(self, patient_history: List[Dict], current_state: Dict, 
        ...
        for t in range(horizon):
            ...
            if hidden is None:
                ...
                biomarkers = current_state.get('biomarkers', torch.zeros(cfg.num_biomarkers, device=encoded.device))
    ...
    def learn_from_response(self, treatment: Dict, response: Dict):
        ...
        if len(self.response_memory) >= 10:
            ...
            # Extract therapeutic tensors handling nested dict structure
            treatment_tensors = []
            for r in recent:
                treatment = r['treatment']
                if isinstance(treatment, dict) and 'therapeutic' in treatment:
                    therapeutic = treatment['therapeutic']
                    if isinstance(therapeutic, dict) and 'therapeutic' in therapeutic:
                        # Nested structure: treatment['therapeutic']['therapeutic']
                        tensor = therapeutic['therapeutic']
                    else:
                        # Direct structure: treatment['therapeutic'] is the tensor
                        tensor = therapeutic
                    
                    if torch.is_tensor(tensor):
                        treatment_tensors.append(tensor.to(device))
                    else:
                        # Fallback to empty tensor
                        treatment_tensors.append(torch.zeros(cfg.hidden_dim, device=device))
            
            if treatment_tensors:
                treatment_features = torch.stack(treatment_tensors).mean(0)
            else:
                treatment_features = torch.zeros(cfg.hidden_dim, device=device)
            
            response_tensors = []
            for r in recent:
                biomarker_change = r['response'].get('biomarker_change')
                if torch.is_tensor(biomarker_change):
                    response_tensors.append(biomarker_change.to(device))
                else:
                    response_tensors.append(torch.zeros(cfg.num_biomarkers, device=device))
            
            if response_tensors:
                response_features = torch.stack(response_tensors).mean(0)
            else:
                response_features = torch.zeros(cfg.num_biomarkers, device=device)
            ...
            # Ensure treatment_features has correct size
            if treatment_features.shape[0] != cfg.hidden_dim:
                # Pad or truncate to correct size
                if treatment_features.shape[0] < cfg.hidden_dim:
                    padded = torch.zeros(cfg.hidden_dim, device=device)
                    padded[:treatment_features.shape[0]] = treatment_features
                    treatment_features = padded
                else:
                    treatment_features = treatment_features[:cfg.hidden_dim]
            
            # Ensure response_features has correct size
            if response_features.shape[0] != cfg.hidden_dim:
                # Pad or truncate to correct size
                if response_features.shape[0] < cfg.hidden_dim:
                    padded = torch.zeros(cfg.hidden_dim, device=device)
                    padded[:response_features.shape[0]] = response_features
                    response_features = padded
                else:
                    response_features = response_features[:cfg.hidden_dim]
    ...
    def _prepare_history(self, patient_history: List[Dict]) -> torch.Tensor:
        ...
        if not patient_history:
            ...
            return torch.zeros(1, 1, cfg.hidden_dim + cfg.num_biomarkers, device=device)
        ...
        for record in patient_history[-24:]:  # Last 24 hours
            ...
            biomarkers = record.get('biomarkers', torch.zeros(cfg.num_biomarkers))
            
            # Handle case where state might be None or empty
            if state is None:
                state = torch.zeros(cfg.hidden_dim)
...
class TherapeuticStemGene(ContinuousDepthGeneModule):
    """Stem gene that can differentiate into any therapeutic function - Maps to S gene (Stem)"""
    
    def __init__(self):
        super().__init__('S', 0)  # Changed from 'TS' to 'S' to match original architecture
        
        # Map to original S gene position (0.5 ± 0.2) - stem cells can go anywhere
        self.position = np.clip(np.random.normal(0.5, 0.2), 0, 1.0)
        
        # Differentiation state (can be partial) - Updated to use original gene types
        # Initialize with device support
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.differentiation_state = nn.Parameter(torch.zeros(5, device=device))  # [V, D, J, Mixed, Stem]
        ...
        self.niche_sensor = nn.Sequential(
            nn.Linear(cfg.num_biomarkers + 10, 128),
            nn.ReLU(),
            nn.Linear(128, 64),
            nn.ReLU(),
            nn.Linear(64, 3)  # Niche scores for V, D, J
        )
    ...
    def sense_therapeutic_needs(self, patient_state: Dict, population_state: Dict) -> Dict:
        ...
        # Count existing therapeutic genes - Using original gene types
        gene_counts = {'V': 0, 'D': 0, 'J': 0}
        gene_performance = {'V': [], 'D': [], 'J': []}
        ...
        need_features = torch.cat([
            ...
            torch.tensor([
                ...
                float(gene_counts['V'] == 0),  # No sensors (V genes)
                float(gene_counts['D'] < 2),    # Few effectors (D genes)
                float(gene_counts['J'] == 0),   # No controller (J genes)
        ...
        return {
            ...
            'recommended_type': ['V', 'D', 'J'][need_scores.argmax()]
        }
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor, 
        ...
        # Weighted combination of therapeutic components - mapped to V/D/J
        h_therapeutic = (
            diff_weights[0] * self.stem_components['sensing'](h) +    # V gene (Biosensor)
            diff_weights[1] * self.stem_components['effector'](h) +   # D gene (Effector)
            diff_weights[2] * self.stem_components['control'](h) +    # J gene (Controller)
            diff_weights[3] * (self.stem_components['sensing'](h) +   # Mixed
                              self.stem_components['effector'](h)) / 2 +
            diff_weights[4] * self.stem_components['universal'](h)    # S gene (Stem)
        )
    ...
    def emergency_response(self, critical_condition: str):
        ...
        emergency_map = {
            'cytokine_storm': 'D',   # Need therapeutic effector (D gene)
            'organ_failure': 'J',     # Need adaptive control (J gene)
            'unknown_pathogen': 'V',  # Need biosensing (V gene)
            'treatment_resistance': 'Mixed'  # Need novel approach
        }
        
        target_type = emergency_map.get(critical_condition, 'D')  # Default to D gene (effector)
