...
class TherapeuticGerminalCenter(ProductionGerminalCenter):
    ...
    def _adapt_to_patient_response(self, response: Dict):
        ...
        if response.get('adverse_event', False):
            ...
            self._guide_stem_differentiation('J')  # Adaptive controllers (J genes)
        elif response['efficacy_score'] < 0.3:
            # Need better effectors
            self._guide_stem_differentiation('D')  # Better effectors (D genes)
...
class LivingTherapeuticSystem:
    ...
    def write_therapeutic_visualization_state(self):
        ...
        zones = {
            ...
            'memoryEliteZone': {'center': [0, 120, 0], 'radius': 60},
            'quantumDisk': {'center': [0, -120, 0], 'radius': 70},
            'mantleZone': {'center': [0, 0, 0], 'radius': 200}
        }

        def get_cell_zone(cell_type):
            if cell_type == 'stem': return 'memoryEliteZone'
            if cell_type == 'effector': return 'lightZone'
            if cell_type == 'biosensor': return 'darkZone'
            if cell_type == 'controller': return 'quantumDisk'
            return 'mantleZone'
        ...
        # --- Pre-process cells to determine specialization based on evolution ---
        all_cell_info = []
        zone_counts = defaultdict(int)
        for cell_id, cell in self.population.items():
            # Determine cell specialization based on:
            # 1. Fitness history (high fitness = memory/elite zone)
            # 2. Recent mutations (high mutation = dark zone)
            # 3. Gene composition balance (balanced = light zone for selection)
            # 4. Quantum genes present (quantum zone)
            
            # Calculate cell metrics
            avg_fitness = np.mean(cell.fitness_history) if cell.fitness_history else 0.5
            recent_mutations = sum(1 for g in cell.genes if hasattr(g, 'transposition_history') 
                                 and len(g.transposition_history) > 0)
            has_quantum = any(hasattr(g, 'quantum_state') or 'Quantum' in type(g).__name__ 
                            for g in cell.genes)
            
            # Gene diversity (not dominance)
            gene_types = set(g.gene_type for g in cell.genes if hasattr(g, 'gene_type'))
            gene_diversity = len(gene_types) / 4.0  # Normalized by max types (V,D,J,S)
            
            # Determine zone based on cell characteristics
            if avg_fitness > 0.8:
                zone_name = 'memoryEliteZone'  # High performing cells
            elif recent_mutations > 2:
                zone_name = 'darkZone'  # Actively mutating cells
            elif has_quantum:
                zone_name = 'quantumDisk'  # Quantum-enabled cells
            elif gene_diversity > 0.6:
                zone_name = 'lightZone'  # Well-balanced cells for selection
            else:
                zone_name = 'mantleZone'  # Transitional cells
            
            # Determine functional type based on actual gene activity, not just count
            active_functions = {'biosensor': 0, 'effector': 0, 'controller': 0, 'stem': 0}
            for gene in cell.genes:
                if gene.is_active and hasattr(gene, 'therapeutic_function'):
                    function = gene.therapeutic_function.get('function', '')
                    if 'sensing' in function or gene.gene_type == 'V':
                        active_functions['biosensor'] += gene.fitness_contribution
                    elif 'delivery' in function or gene.gene_type == 'D':
                        active_functions['effector'] += gene.fitness_contribution
                    elif 'control' in function or gene.gene_type == 'J':
                        active_functions['controller'] += gene.fitness_contribution
                    elif gene.gene_type == 'S':
                        active_functions['stem'] += 1
            
            # Cell type based on function contribution, not count
            if sum(active_functions.values()) > 0:
                cell_type = max(active_functions, key=active_functions.get)
            else:
                cell_type = 'balanced'
        ...
        for info in all_cell_info:
            ...
            for gene in cell.genes:
                if gene.is_active and hasattr(gene, 'gene_type'):
                    # Use the actual therapeutic gene type (BS, TE, AC, TS)
                    therapeutic_gene_type = gene.gene_type
                    type_counts[therapeutic_gene_type] += 1
                    gene_list.append({
                        'gene_id': str(id(gene)),
                        'gene_type': therapeutic_gene_type,
                        'type': therapeutic_gene_type,
                        'is_active': True,
            ...
            cell_data = {
                ...
                'type': cell_type,
                'therapeutic_genes': {
                    'biosensor': type_counts.get('V', 0),
                    'effector': type_counts.get('D', 0),
                    'controller': type_counts.get('J', 0),
                    'stem': type_counts.get('S', 0)
                },
        ...
        # This ensures the JSON is written to the correct location regardless of where the script is run from
        current_file_dir = os.path.dirname(os.path.abspath(__file__))
        viz_dir = os.path.join(current_file_dir, 'therapeutics_viz')
    ...
    def _inject_enhanced_stem_cells(self, count: int = 10):
        ...
        for cell_id, cell in list(self.population.items())[:count]:
            if len(cell.genes) < 10:  # Max genes per cell
                ...
                stem = StemGeneModule(gene_types=['V', 'D', 'J', 'S'])  # Use original gene types
                ...
                # Initialize with original gene position preferences
                stem.position_preferences = {
                    'V': (0.15, 0.1),   # V genes (biosensors) near input
                    'D': (0.45, 0.1),   # D genes (effectors) in middle
                    'J': (0.8, 0.1),    # J genes (controllers) near output
                    'S': (0.5, 0.3)     # S genes (stems) anywhere
                }
        ...
        for cell in self.population.values():
            for gene in cell.genes:
                if isinstance(gene, StemGeneModule) and gene.commitment_level < 0.3:
                    population_stats = {
                        'V_count': sum(1 for c in self.population.values() 
                                      for g in c.genes if isinstance(g, BiosensorGene)),
                        'D_count': sum(1 for c in self.population.values() 
                                      for g in c.genes if isinstance(g, TherapeuticEffectorGene)),
                        'J_count': sum(1 for c in self.population.values() 
