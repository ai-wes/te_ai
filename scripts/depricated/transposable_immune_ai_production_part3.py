...
class SelfModifyingArchitecture(nn.Module):
    ...
    def decide_modification(self, performance_metrics: Dict[str, float],
        ...
        grad_stats = self._compute_gradient_statistics()
        ...
        if meta_input.shape[-1] < self.base_dim * 3:
            padding = torch.zeros(1, 1, self.base_dim * 3 - meta_input.shape[-1]).to(meta_input.device)
            meta_input = torch.cat([meta_input, padding], dim=-1)
        
        # Get modification decision
        output, _ = self.meta_controller(meta_input)
        ...
        return ArchitectureModification(mod_type, params['target'], params)
    ...
    def _resize_layer(self, params: Dict) -> bool:
        ...
        if not hasattr(module[0], 'in_features'):
            return False
        
        old_in = module[0].in_features
        old_out = module[0].out_features
        ...
        new_layer = nn.Linear(old_in, new_size).to(module[0].weight.device)
        
        # Initialize intelligently
        with torch.no_grad():
            if new_size > old_out:
                # Expanding: copy old weights and add noise to new
                new_layer.weight.data[:old_out] = module[0].weight.data
                new_layer.weight.data[old_out:] = torch.randn(
                    new_size - old_out, old_in
                ).to(new_layer.weight.device) * 0.01
                new_layer.bias.data[:old_out] = module[0].bias.data
            else:
                # Contracting: use PCA or importance sampling
                weight_importance = module[0].weight.data.norm(dim=1)
                _, indices = torch.topk(weight_importance, new_size)
                new_layer.weight.data = module[0].weight.data[indices]
                new_layer.bias.data = module[0].bias.data[indices]
