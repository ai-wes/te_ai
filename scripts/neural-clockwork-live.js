...
class GerminalCenterLayout {
  ...
  mantleZonePosition(cellData, index) {
    // Flowing arrangement for transitional cells
    const zone = this.zones.mantleZone;

    // Toroidal arrangement
    const majorRadius = zone.radius * 0.7;
    const minorRadius = zone.radius * 0.3;

    const u = (index * 0.1) % (2 * Math.PI);
    const v = (index * 0.618) % (2 * Math.PI);
