class TransposableNeuralCell {
  ...
  createStormCloud(scene, zone) {
    // Create simple box for storm cloud core
    const stormGroup = new THREE.Group();

    // Simple box geometry for storm core
    const coreGeometry = new THREE.BoxGeometry(20, 30, 20);
  ...
  createCosmicParticles(scene, zone) {
    // Create simple ring shapes instead of particles
    const orbitalGroup = new THREE.Group();

    // Create 3 simple rings at different sizes
    for (let i = 0; i < 3; i++) {
      const radius = zone.innerRadius + (i + 1) * 5;
      const ringGeometry = new THREE.TorusGeometry(radius, 1, 4, 8);
  ...
  createCyclonicFormation(scene, center, radius, type) {
    // Create simple disk instead of particle rings
    const diskGeometry = new THREE.CylinderGeometry(radius, radius, 5, 8);
    ...
    this.zoneGroup.add(disk);

    // Add simple core sphere
    const coreGeometry = new THREE.SphereGeometry(5, 8, 8);
    const coreMaterial = new THREE.MeshBasicMaterial({
      color: 0xff00ff,
      transparent: true,
      opacity: 0.8
  ...
  createEnergyFlow(scene, zone) {
    // Create simple glowing spheres instead of particles
    const sphereCount = 5;
    
    for (let i = 0; i < sphereCount; i++) {
      const angle = (i / sphereCount) * Math.PI * 2;
      const radius = zone.radius * 0.7;
      const height = ((i / sphereCount) - 0.5) * zone.height;
...
class QuantumEntanglementVisualizer {
  ...
  createQuantumField() {
    // Simple quantum field sphere
    const fieldGeometry = new THREE.SphereGeometry(200, 8, 6);
...
class GerminalCenterLayout {
  ...
  createZoneVisualizations(scene) {
    ...
    sortedZones.forEach(([name, zone]) => {
      ...
      const ringGeometry = new THREE.TorusGeometry(zone.radius, 1, 4, 8); // Minimal segments
      const ringMaterial = new THREE.MeshBasicMaterial({
        color: zone.color,
        transparent: true,
        opacity: 0.3,
  ...
  createZoneParticles(scene, zone) {
    // Create simple small cubes instead of particles
    const cubeCount = 8;
    
    for (let i = 0; i < cubeCount; i++) {
      const theta = (i / cubeCount) * Math.PI * 2;
      ...
      cube.position.set(
        zone.center.x + Math.cos(theta) * r,
        zone.center.y,
        zone.center.z + Math.sin(theta) * r
      );
      cube.frustumCulled = false;
      this.zoneGroup.add(cube);
    }
  }
}
...
class LiveNeuralClockwork {
  constructor() {
    ...
    // Speed mode flag - disable constant animations
    this.speedMode = true;
  ...
  createCellMechanism(cellData) {
    ...
    switch (specialization.primary) {
      ...
      case "D":
        ...
        hubGeometry = new THREE.SphereGeometry(cellSize, 8, 6);
        break;
      case "J":
  ...
  determineCellSpecialization(cellData) {
    ...
    // Special handling for stem cells - they need to be DOMINATED by S genes
    if (geneTypes.S > 0 && geneTypes.S > totalTypedGenes * 0.4) {
      primary = "S";
      maxCount = geneTypes.S;
      console.log(
        `Cell ${cellData.cell_id} has ${geneTypes.S} S genes (${(
          (geneTypes.S / totalTypedGenes) *
          100
        ).toFixed(1)}%) - classified as STEM CELL`
      );
    } else {
  ...
  addGeneIndicators(cellGroup, cellData, cellSize) {
    ...
    cellData.genes.forEach((gene, index) => {
      ...
      const indicatorSize = 0.15 + gene.depth * 0.05; // Much smaller (was 0.5 + depth * 0.2)
      let geometry;
      ...
      switch (gene.gene_type) {
        ...
        case "Q":
          geometry = new THREE.TorusKnotGeometry(
            indicatorSize * 0.7,
            indicatorSize * 0.2,
            8,  // Reduced segments (was 16)
            4   // Reduced segments (was 8)
          );
          break;
        case "S":
      ...
      const radius = cellSize + 1 + gene.position * 0.5; // Tighter orbit (was + 2 + position * 2)
      const height = Math.sin(index * 0.5) * 1; // Less vertical spread (was 2)
  ...
  createAngelicStemCell(cellData, cellSize) {
    ...
    const orbGeometry = new THREE.SphereGeometry(cellSize * 0.8, 8, 6);
    ...
    for (let i = 0; i < 3; i++) {
      const haloRadius = cellSize * (1.1 + i * 0.15); // Tighter halos (was 1.2 + i * 0.3)
      const haloGeometry = new THREE.TorusGeometry(haloRadius, 0.05, 4, 8); // Thinner halos (was 0.1)
  ...
  addDivineAura(cellGroup, cellSize) {
    // Create simple glow sphere instead of particles
    const auraGeometry = new THREE.SphereGeometry(cellSize * 2.5, 6, 6);
  ...
  createQuantumEmergence(position) {
    // Create expanding spheres instead of particles
    const sphereCount = 8;
    const spheres = [];
    ...
    for (let i = 0; i < sphereCount; i++) {
      ...
      const sphereMaterial = new THREE.MeshBasicMaterial({
        color: new THREE.Color(0.5 + i/sphereCount * 0.5, 0.8 + i/sphereCount * 0.2, 1.0),
        transparent: true,
        opacity: 1.0
      });
    ...
    const animate = () => {
      const elapsed = (Date.now() - startTime) / 1000;
      if (elapsed > 5) {
        spheres.forEach(sphere => this.scene.remove(sphere));
        return;
      }

      spheres.forEach((sphere, i) => {
        const theta = (i / sphereCount) * Math.PI * 2;
        const speed = 15;
        const radius = elapsed * speed;

        sphere.position.x = position.x + Math.cos(theta) * radius;
        sphere.position.z = position.z + Math.sin(theta) * radius;
        sphere.position.y = position.y;
        
        sphere.material.opacity = 1.0 - elapsed / 5;
        sphere.scale.setScalar(1 + elapsed * 0.5);
      });
  ...
  createQuantumRipple(position) {
    // Create simple expanding torus
    const ringGeometry = new THREE.TorusGeometry(5, 0.5, 4, 8);
    const ringMaterial = new THREE.MeshBasicMaterial({
      color: 0xff00ff,
      transparent: true,
      opacity: 1.0
  ...
  emitQuantumParticles(position) {
    // Create simple spiraling cubes instead of particles
    const cubeCount = 8;
    const cubes = [];
    ...
    const animate = () => {
      const elapsed = (Date.now() - startTime) / 1000;
      if (elapsed > 4) {
        cubes.forEach(cube => this.scene.remove(cube));
        return;
      }

      cubes.forEach((cube, i) => {
        const t = i / cubeCount;
        const angle = t * Math.PI * 2 + elapsed * 2;
        const radius = elapsed * 10;
        const height = (t - 0.5) * 20 * elapsed;

        cube.position.x = position.x + Math.cos(angle) * radius;
        cube.position.y = position.y + height;
        cube.position.z = position.z + Math.sin(angle) * radius;
        
        cube.material.opacity = 1.0 - elapsed / 4;
  ...
  addFitnessAura(cellGroup, fitness, cellSize) {
    // Create simple glow rings based on fitness level
    const ringCount = Math.floor(fitness * 3) + 1;
    if (ringCount === 0) return;

    const auraGroup = new THREE.Group();

    for (let i = 0; i < ringCount; i++) {
      const ringRadius = cellSize + (i + 1) * 0.5; // Much closer spacing (was * 2)
      const ringGeometry = new THREE.TorusGeometry(ringRadius, 0.05, 4, 8); // Much thinner (was 0.2)
    ...
    cellGroup.add(auraGroup);
    cellGroup.userData.fitnessAura = auraGroup;
  }
  ...
  updateCellStructure(data) {
    ...
    if (data.genes) {
      data.genes.forEach((geneData, i) => {
        ...
        const radius = 8 + (geneData.position || i) * 0.8; // Much tighter orbit (was 15 + i * 2)
        const height = Math.sin(angle * 2) * 2; // Less vertical spread (was 5)
  ...
  animate() {
    ...
    // Skip all constant animations in speed mode for better performance
