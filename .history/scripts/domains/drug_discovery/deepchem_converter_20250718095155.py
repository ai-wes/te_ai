...
class DeepChemToTEAI:
    ...
    def convert_molnet_dataset(
        ...
        # Load the dataset with our robust featurizer
        logger.info(f"Loading {dataset_name_str} dataset with robust featurization...")
        ...
        try:
            ...
            logger.warning(f"Failed to load dataset with robust featurizer: {e}")
            logger.info("Trying with simpler featurizer...")
            
            # Fallback to simpler approach
            try:
                from deepchem.feat import CircularFingerprint
                simple_featurizer = CircularFingerprint(size=1024)
                tasks, datasets, transformers = loader_fn(
                    featurizer=simple_featurizer,
                    splitter=splitter,
                    data_dir=data_dir
                )
                train_data, valid_data, test_data = datasets
            except Exception as e2:
                logger.error(f"Failed to load dataset with simple featurizer too: {e2}")
                raise RuntimeError(f"Could not load {dataset_name_str} dataset with any featurizer")
...
class RobustFeaturizer(Featurizer):
    ...
    def _featurize(self, datapoint: Any, **kwargs) -> np.ndarray:
        ...
        try:
            ...
            if hasattr(self.base_featurizer, '_featurize'):
                features = self.base_featurizer._featurize(datapoint, **kwargs)
            else:
                # Fallback to the main featurize method
                features = self.base_featurizer.featurize([datapoint], **kwargs)[0]
