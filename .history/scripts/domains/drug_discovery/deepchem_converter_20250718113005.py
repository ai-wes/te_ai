...
class DeepChemToTEAI:
    ...
    def convert_molnet_dataset(
        ...
        """
        Load and convert a MoleculeNet dataset with a guaranteed-to-work,
        robust featurization pipeline.
        """
        logger.info(f"Loading {dataset_name} from MoleculeNet...")

        # --- DIRECT FIX: Explicitly use CircularFingerprint ---
        # We know MorganGenerator is not available in this environment.
        # We will directly use CircularFingerprint, which is the older equivalent.
        
        expected_size = 1024
        try:
            from deepchem.feat import CircularFingerprint
            base_featurizer = CircularFingerprint(size=expected_size)
            featurizer_name = "CircularFingerprint"
            logger.info("Using CircularFingerprint as the base featurizer.")
        except ImportError as e:
            logger.critical(f"FATAL: Even CircularFingerprint could not be imported from DeepChem: {e}")
            raise RuntimeError("Critical DeepChem featurizers are missing. Please check your installation.") from e

        # Wrap it in our sanitizing featurizer to handle bad SMILES
        sanitizing_wrapper = SanitizingFeaturizer(
            base_featurizer=base_featurizer,
            expected_size=expected_size
        )
        logger.info(f"Using SanitizingFeaturizer wrapping {featurizer_name}.")

        # --- The robust pre-filtering pipeline remains the same ---
...
class SanitizingFeaturizer(Featurizer):
    """
