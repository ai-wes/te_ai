...
class DeepChemToTEAI:
    ...
    def convert_molnet_dataset(
        ...
        if featurizer == 'ECFP':
            try:
                # Use RDKit's MorganGenerator directly
                from rdkit.Chem import rdFingerprintGenerator
                fp_gen = rdFingerprintGenerator.GetMorganGenerator(radius=2, fpSize=expected_size)
                # Create a wrapper that works with DeepChem's featurizer interface
                from deepchem.feat import CircularFingerprint
                base_featurizer = CircularFingerprint(size=expected_size, radius=2)
                featurizer_name = "MorganGenerator (via RDKit)"
                logger.info("Successfully initialized MorganGenerator via RDKit.")
            except (ImportError, Exception) as e:
                logger.warning(f"Could not import or initialize MorganGenerator via RDKit: {e}. Falling back to CircularFingerprint.")
                try:
                    # Fallback to the older, but still effective, featurizer
                    from deepchem.feat import CircularFingerprint
                    base_featurizer = CircularFingerprint(size=expected_size)
                    featurizer_name = "CircularFingerprint"
                    logger.info("Successfully initialized CircularFingerprint as fallback.")
                except (ImportError, Exception) as e2:
                    logger.error(f"FATAL: Could not import any fingerprint featurizer: {e2}")
