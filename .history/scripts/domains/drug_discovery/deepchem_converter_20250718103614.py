...
class DeepChemToTEAI:
    ...
    def convert_molnet_dataset(
        ...
        if featurizer == 'ECFP':
            try:
                # Use RDKit's MorganGenerator directly as requested
                from rdkit.Chem import rdFingerprintGenerator
                fp_gen = rdFingerprintGenerator.GetMorganGenerator(radius=2, fpSize=expected_size)
                # Create a custom featurizer that uses RDKit's MorganGenerator
                class RDKitMorganFeaturizer(Featurizer):
                    def __init__(self, fp_gen):
                        self.fp_gen = fp_gen
                        super().__init__()
                    ...
                    def _featurize(self, datapoint, **kwargs):
                        ...
                        try:
                            ...
                            arr = np.zeros(expected_size, dtype=float)
                ...
                base_featurizer = RDKitMorganFeaturizer(fp_gen)
                featurizer_name = "RDKit MorganGenerator"
                logger.info("Successfully initialized RDKit MorganGenerator.")
            except (ImportError, Exception) as e:
                logger.error(f"FATAL: Could not import RDKit MorganGenerator: {e}")
        ...
        data_dir = cfg.deepchem_data_dir
        logger.info(f"Using DeepChem data directory: {data_dir}")
        
        # Debug: Check if the data directory exists and contains the expected files
        import os
        if os.path.exists(data_dir):
            logger.info(f"Data directory exists: {data_dir}")
            bbbp_dir = os.path.join(data_dir, "bbbp")
            if os.path.exists(bbbp_dir):
                logger.info(f"BBBP dataset directory exists: {bbbp_dir}")
                files = os.listdir(bbbp_dir)
                logger.info(f"Files in BBBP directory: {files}")
            else:
                logger.warning(f"BBBP dataset directory not found: {bbbp_dir}")
        else:
            logger.warning(f"Data directory does not exist: {data_dir}")
            logger.info("Falling back to DeepChem's default data directory")
            data_dir = dc.utils.get_data_dir()
            logger.info(f"Using DeepChem default data directory: {data_dir}")
        
        # 1. Load raw data with our robust featurizer from the start
        loader_fn = getattr(dc.molnet, f'load_{dataset_name.lower()}')
        tasks, (raw_dataset,), transformers = loader_fn(
            featurizer=sanitizing_wrapper,  # Use our robust featurizer from the start
            splitter=None,
            data_dir=data_dir
        )

        # 2. Filter out invalid molecules before splitting (ScaffoldSplitter needs valid SMILES)
        logger.info("Step 2: Filtering out invalid molecules before splitting...")
        valid_indices = []
        for i, smiles in enumerate(raw_dataset.ids):
            mol = Chem.MolFromSmiles(smiles)
            if mol is not None:
        ...
        if invalid_count > 0:
            logger.warning(f"Removed {invalid_count} invalid SMILES strings before splitting.")
        
        # Create a cleaned dataset with only valid molecules
        cleaned_dataset = raw_dataset.select(valid_indices)
        
        # 3. Now split the cleaned dataset
        logger.info("Step 3: Splitting the cleaned dataset...")
        splitter_fn = dc.splits.ScaffoldSplitter()
        train_data, valid_data, test_data = splitter_fn.train_valid_test_split(cleaned_dataset)
        
        logger.info("Step 4: Applying data transformations...")
