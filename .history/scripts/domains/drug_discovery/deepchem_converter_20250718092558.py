    ...
    def convert_molnet_dataset(
        ...
        if featurizer == 'ECFP':
            try:
                ...
                logger.warning(f"Could not import or initialize MorganGenerator from DeepChem: {e}. Falling back to a simple CircularFingerprint featurizer.")
                try:
                    from deepchem.feat import CircularFingerprint
                    fallback_featurizer = CircularFingerprint(size=1024)
                    featurizer_obj = RobustFeaturizer(
                        base_featurizer=fallback_featurizer,
                        expected_size=1024
                    )
                    logger.info("Using RobustFeaturizer with CircularFingerprint as fallback.")
                except Exception as e2:
                    logger.error(f"Could not import CircularFingerprint from DeepChem either: {e2}. Using featurizer string name as last resort.")
