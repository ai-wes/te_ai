...
from scripts.domains.drug_discovery.robust_featurizer import RobustFeaturizer
from rdkit import Chem
    ...
    def convert_molnet_dataset(
        ...
        if featurizer == 'ECFP':
            try:
                ...
                featurizer_obj = RobustFeaturizer(base_featurizer=morgan_featurizer, expected_size=1024)
        ...
        # Load the raw data first without featurizing or splitting
        loader_fn = getattr(cfg.deepchem_data_dir, f'load_{dataset_name.lower()}')
        tasks, (dataset,), transformers = loader_fn(featurizer=dc.feat.DummyFeaturizer(), splitter=None, data_dir=data_dir)
        ...
        if invalid_smiles_count > 0:
            logger.warning(f"Removed {invalid_smiles_count} invalid SMILES strings from the '{dataset_name}' dataset.")
        
        # Create a new, cleaned dataset
        cleaned_dataset = dataset.select(valid_indices)

        # *** NOW, PROCEED WITH THE CLEANED DATASET ***
        # 1. Featurize the cleaned data
        logger.info("Featurizing the cleaned dataset...")
        cleaned_dataset = cleaned_dataset.transform(lambda ds: ds.featurize(featurizer_obj), out_dir=None)

        # 2. Split the featurized, cleaned data
        logger.info("Splitting the cleaned dataset...")
        splitter_fn = dc.splits.ScaffoldSplitter()
        train_data, valid_data, test_data = splitter_fn.train_valid_test_split(cleaned_dataset)
        
        # 3. Transform the data (e.g., normalization)
        ...
        logger.info(f"Converted {dataset_name}:")
