...
class RobustFeaturizer(Featurizer):
    ...
    def _featurize(self, datapoint: Any, **kwargs) -> np.ndarray:
        ...
        try:
            ...
            # Check if the output is valid and has the correct shape
            if isinstance(features, np.ndarray) and features.shape == (self.expected_size,):
                return features
            else:
                # Log a warning if the shape is incorrect
                logger.warning(f"Featurization produced an unexpected shape for SMILES: {datapoint}. "
                               f"Expected ({self.expected_size},), got {features.shape}. Returning zero vector.")
