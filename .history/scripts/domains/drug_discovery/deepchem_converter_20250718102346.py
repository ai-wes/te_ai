...
class DeepChemToTEAI:
    ...
    def convert_molnet_dataset(
        ...
        if featurizer == 'ECFP':
            try:
                # Try the modern, recommended featurizer first
                ...
                logger.warning(f"Could not import or initialize MorganGenerator: {e}. Falling back to CircularFingerprint.")
                try:
                    ...
                    logger.error(f"FATAL: Could not import any fingerprint featurizer from DeepChem: {e2}")
