...
# --- Import our robust featurizer wrapper ---
...
class DeepChemToTEAI:
    ...
    def convert_molnet_dataset(
        ...
        """
        Load and convert a MoleculeNet dataset with a robust, sanitizing featurizer.
        This version removes fallbacks to ensure the correct featurizer is always used.
        """
        logger.info(f"Loading {dataset_name} from MoleculeNet...")

        # --- HARDENED FEATURIZER SETUP ---
        # We will now explicitly build our robust featurizer and fail if we can't.
        expected_size = 1024  # Default for ECFP/Morgan
        if featurizer == 'ECFP':
            try:
                from deepchem.feat import MorganGenerator
                base_featurizer = MorganGenerator(radius=2, size=expected_size)
                featurizer_name = "MorganGenerator"
                logger.info("Successfully initialized MorganGenerator.")
            except (ImportError, Exception) as e:
                logger.critical(f"FATAL: Could not import or initialize MorganGenerator from DeepChem: {e}")
                logger.critical("This featurizer is a critical dependency. Please check your DeepChem installation.")
                raise RuntimeError("Failed to create the required featurizer.") from e
        else:
            # If you plan to use other featurizers, you would add their robust setup here.
            # For now, we only support ECFP for this benchmark.
        ...
        logger.info(f"Using SanitizingFeaturizer wrapping {featurizer_name}.")

        # --- The rest of the pipeline remains the same robust version ---
        # Use DeepChem's default data directory if not specified in config
        data_dir = getattr(cfg, "deepchem_data_dir", None)
        if not data_dir:
            data_dir = dc.utils.get_data_dir()
        logger.info(f"Using DeepChem data directory: {data_dir}")

        # --- PRE-FILTERING STEP (Still the most robust approach) ---
        logger.info("Step 1: Loading raw data with DummyFeaturizer for cleaning...")
        ...
        try:
            ...
            logger.critical(f"FATAL: Failed to load raw data for {dataset_name}. Error: {e}")
            raise

        # 2. Filter out invalid molecules that RDKit cannot parse
        logger.info("Step 2: Filtering out invalid SMILES strings...")
        valid_indices = []
        ...
        # 3. Featurize the now-guaranteed-clean data using our sanitizing wrapper
        logger.info("Featurizing the cleaned dataset...")
        featurized_dataset = cleaned_dataset.featurize(sanitizing_wrapper)

        # 4. Split the data
        logger.info("Splitting the featurized dataset...")
