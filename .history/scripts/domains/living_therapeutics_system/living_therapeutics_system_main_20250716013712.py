...
class LivingTherapeuticSystem:
    ...
    def write_therapeutic_visualization_state(self):
        ...
        state = {
            ...
            'timestamp': time.time(),
            'patient': {
                'id': self.patient_profile.get('patient_id', 'Unknown'),
                'disease': self.patient_profile.get('disease_type', 'Unknown'),
        ...
        # Add cell data with therapeutic-specific information
        for cell_id, cell in list(self.population.items())[:100]:  # Limit to 100 cells for performance
            # Determine cell's therapeutic type based on its genes
            cell_type = 'balanced'
            biosensor_count = 0
            effector_count = 0
            controller_count = 0
            ...
            max_count = max(biosensor_count, effector_count, controller_count, stem_count)
            if max_count > 0:
                if biosensor_count == max_count:
                    cell_type = 'biosensor'
                elif effector_count == max_count:
                    cell_type = 'effector'
                elif controller_count == max_count:
                    cell_type = 'controller'
                elif stem_count == max_count:
                    cell_type = 'stem'
            
            # Convert genes to visualization format
            gene_list = []
            for gene in cell.genes:
                if gene.is_active and hasattr(gene, 'gene_type'):
                    # Directly use the gene's defined type (BS, TE, AC, TS)
                    gene_type = gene.gene_type
                    
                    gene_list.append({
                        'gene_id': str(id(gene)),
                        'gene_type': gene_type,
                        'type': gene_type,  # Ensure both fields have the correct type
                        'is_active': True,
                        'position': gene.position if hasattr(gene, 'position') else 0,
                        'depth': gene.compute_depth().item() if hasattr(gene, 'compute_depth') else 1.0,
                        'is_quantum': hasattr(gene, 'quantum_state') and gene.quantum_state is not None
                    })
            ...
            cell_data = {
                ...
                'type': cell_type,
                'therapeutic_genes': {
                    'biosensor': biosensor_count,
                    'effector': effector_count,
        ...
        if self.treatment_history:
            ...
            state['therapeutic_metrics'] = {
                'efficacy': latest_treatment.get('efficacy', 0),
                'safety': latest_treatment.get('safety', 1.0),
                'resistance': latest_treatment.get('resistance', 0)
            }
        
        # Add visualization-friendly data similar to core system
        state['stress_level'] = self.germinal_center.current_stress
        state['stress'] = self.germinal_center.current_stress  # Some parts expect 'stress'
        state['mean_fitness'] = np.mean([cell.fitness_history[-1] if cell.fitness_history else 0.5 
                                        for cell in self.population.values()])
        state['fitness'] = state['mean_fitness']  # Alias for compatibility
        state['diversity'] = len(set(type(gene).__name__ for cell in self.population.values() 
                                    for gene in cell.genes if gene.is_active)) / 4.0
        state['phase'] = getattr(self.germinal_center, 'phase', 'stable')
        state['nodes'] = []  # Could add therapeutic-specific nodes
        state['links'] = []  # Could add therapeutic-specific links
        
        # Write to therapeutic-specific JSON file in viz directory
        therapeutic_json_path = os.path.join(
            os.path.dirname(__file__), 
            'therapeutics_viz',
            'therapeutic_visualization_state.json'
        )
        
        # Create directory if it doesn't exist
        os.makedirs(os.path.dirname(therapeutic_json_path), exist_ok=True)
