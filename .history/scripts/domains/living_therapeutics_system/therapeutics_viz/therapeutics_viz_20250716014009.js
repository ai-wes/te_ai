class TransposableNeuralCell {
  constructor() {
    ...
    this.zones = {
      ...
      darkZone: {
        ...
        center: new THREE.Vector3(0, 0, 0),
        innerRadius: 5, // Storm cloud core
        outerRadius: 30, // Storm cloud extent
        height: 60, // Vertical extent
  ...
  centralCylinderPosition(cellData, index) {
    ...
    // This function returns the position for a cell, but the visual appearance of the cylinder
    // (black, semi-translucent, glowing purple bloom) is handled in the createCentralCylinder method.
    // Here, we keep the position logic for arranging cells around the cylinder.

    const zone = this.zones.centralCylinder;
    const height = (index / 20) * zone.height - zone.height / 2;
    const angle = (index / 5) * Math.PI * 2;

    // Arrange cells in a spiral around the cylinder's surface
    const x = zone.center.x + Math.cos(angle) * zone.radius * 0.8;
    const z = zone.center.z + Math.sin(angle) * zone.radius * 0.8;
    const y = zone.center.y + height;

    return new THREE.Vector3(x, y, z);
  }

  // --- Add or update the central cylinder mesh to be black, semi-translucent, and glowing purple bloom ---
  // Call this in your scene setup or zone creation logic
  createCentralCylinder(scene) {
    const zone = this.zones.centralCylinder;

    // Remove previous cylinder if it exists
    if (this.centralCylinderMesh) {
      this.zoneGroup.remove(this.centralCylinderMesh);
    }

    // Create a black, semi-translucent cylinder
    ...
    // Black, semi-translucent, with purple emissive for bloom
    const cylinderMaterial = new THREE.MeshPhysicalMaterial({
      ...
      metalness: 0.7,
      transmission: 0.2,
      emissive: 0x9900ff, // Purple glow
      emissiveIntensity: 4.5, // Strong for bloom
      clearcoat: 1.0,
    ...
    // Optionally, add a subtle purple glow using a sprite or mesh for extra bloom
    ...
    glowMesh.position.copy(zone.center);
    glowMesh.frustumCulled = false;

    this.centralCylinderMesh = cylinder;
    this.zoneGroup.add(cylinder);
    this.zoneGroup.add(glowMesh);
  }
  ...
  determineZone(cellData) {
    ...
    // Quantum cells go to quantum disks (top/bottom)
    if (hasQuantumGenes) {
      return "quantumDisk";
    }

    // High stress/mutation cells go to dark zone (storm cloud center only)
    if (stressLevel > 0.7 || this.calculateMutationRate(cellData) > 0.8) {
      return "darkZone";
    }

    // Stem cells and active selection cells go to light zone (gold helix)
    const hasStemGenes =
      cellData.genes?.some((g) => g.gene_type === "S") || false;
    if (hasStemGenes || (fitness > 0.3 && fitness < 0.8)) {
      return "lightZone";
    }

    // Elite memory cells go to memory zone (sphere around light zone)
    if (fitness > 0.8 && generation > 3) {
      return "memoryEliteZone";
    }

    // Everything else goes to mantle zone (outer sphere)
    return "mantleZone";
  }
  ...
  createStormCloud(scene, zone) {
    ...
    for (let i = 0; i < stormCount; i++) {
      ...
      if (flashType > 0.8) {
        ...
        particleColor = 0x444488; // Lighter dark storm color
      }
    ...
    // Add dense core cloud for storm effect
    ...
    core.position.copy(zone.center);
...
class GerminalCenterLayout {
  ...
  determineZone(cellData) {
    const fitness = cellData.fitness || 0.5;
    const hasTherapeuticGenes = cellData.type && cellData.type !== "balanced";

    // Therapeutic-specific zone assignment
    if (cellData.type === "stem") {
      return "memoryZone"; // Stem cells go to memory zone
    }

    if (cellData.type === "effector" && fitness > 0.6) {
      return "lightZone"; // Active effectors in light zone
    }

    if (cellData.type === "biosensor") {
      return "darkZone"; // Sensors in dark zone for mutation
    }

    if (cellData.type === "controller") {
      return "quantumLayer"; // Controllers in quantum layer
    }

    // Default to mantle zone
    return "mantleZone";
  }
  ...
  positionCell(cellData, index, population) {
    ...
    // Create position within zone
    const angle = (index / population.length) * Math.PI * 2;
    const radius = Math.random() * zoneData.radius * 0.8;
    const height = (Math.random() - 0.5) * zoneData.radius * 0.5;

    return new THREE.Vector3(
      zoneData.center.x + Math.cos(angle) * radius,
      zoneData.center.y + height,
      zoneData.center.z + Math.sin(angle) * radius
    );
  }
...
class LiveNeuralClockwork {
  ...
  async startPolling() {
    ...
    const poll = async () => {
      ...
      try {
        // This is therapeutic visualization, always use therapeutic JSON
        const jsonPath = "therapeutic_visualization_state.json?t=" + Date.now();
  ...
  updatePopulationStructure(state) {
    ...
    const zoneCounts = {
      darkZone: 0,
      lightZone: 0,
      memoryZone: 0, // CORRECTED
      quantumLayer: 0, // CORRECTED
      mantleZone: 0,
    };
    ...
    if (state.cells && state.cells.length > 0) {
      ...
      state.cells.forEach((cellData, index) => {
        ...
        if (index === 0) {
          ...
          console.log("Type of cellData.genes:", typeof cellData.genes);
          console.log(
            "Is cellData.genes an array?",
            Array.isArray(cellData.genes)
          );
          console.log("cellData.genes value:", cellData.genes);
        ...
        if (!cell) {
          ...
          console.log(`Created cell ${cellId} at position:`, cell.position);
        ...
        // Determine zone and count
        const zone = this.layout.determineZone(cellData);
        if (zoneCounts[zone] !== undefined) {
          zoneCounts[zone]++;
        }

        // Get sophisticated position based on cell properties
        const targetPosition = this.layout.positionCell(
          cellData,
          index,
          state.cells
        );
      ...
      this.updateZoneCounts(zoneCounts);
    } else {
  ...
  updateZoneCounts(zoneCounts) {
    // Update HTML elements with zone counts
    const darkZoneEl = document.getElementById("darkZoneCount");
    const lightZoneEl = document.getElementById("lightZoneCount");
    const memoryZoneEl = document.getElementById("memoryZoneCount");
    const quantumZoneEl = document.getElementById("quantumZoneCount");
    const mantleZoneEl = document.getElementById("mantleZoneCount");

    if (darkZoneEl) darkZoneEl.textContent = `(${zoneCounts.darkZone})`;
    if (lightZoneEl) lightZoneEl.textContent = `(${zoneCounts.lightZone})`;
    if (memoryZoneEl) memoryZoneEl.textContent = `(${zoneCounts.memoryZone})`; // CORRECTED
    if (quantumZoneEl)
      quantumZoneEl.textContent = `(${zoneCounts.quantumLayer})`; // CORRECTED
    if (mantleZoneEl) mantleZoneEl.textContent = `(${zoneCounts.mantleZone})`;
  }
  ...
  createCellMechanism(cellData) {
    ...
    const minSize = 0.5; // Minimum size for cells with few genes
    const maxSize = 3; // Maximum size for cells with many genes
    ...
    switch (specialization.primary) {
      case "V":
      case "biosensor":
        // V-specialized/biosensor cells are more angular (sensing)
        hubGeometry = new THREE.IcosahedronGeometry(cellSize, 0);
        break;
      case "D":
      case "effector":
        // D-specialized/effector cells are more rounded (treatment delivery)
        hubGeometry = new THREE.SphereGeometry(cellSize, 32, 16);
        break;
      case "J":
      case "controller":
        // J-specialized/controller cells are elongated (coordination)
  ...
  determineCellSpecialization(cellData) {
    // For therapeutic visualization, cell type is provided directly
    let primary = cellData.type || "balanced";

    // Count therapeutic genes if provided
    const therapeuticGeneTypes = {
      biosensor: 0,
      effector: 0,
      controller: 0,
      stem: 0,
    ...
    // Handle genes as array or number for backward compatibility
    let totalGenes = 0;
    let activeGenes = 0;

    if (Array.isArray(cellData.genes)) {
      totalGenes = cellData.genes.length;
      activeGenes = cellData.genes.filter((g) => g.is_active).length;
    } else if (typeof cellData.genes === "number") {
      totalGenes = cellData.genes;
      activeGenes = totalGenes; // All therapeutic genes are considered active
    }

    if (cellData.therapeutic_genes) {
      therapeuticGeneTypes.biosensor =
        cellData.therapeutic_genes.biosensor || 0;
      therapeuticGeneTypes.effector = cellData.therapeutic_genes.effector || 0;
      therapeuticGeneTypes.controller =
        cellData.therapeutic_genes.controller || 0;
      therapeuticGeneTypes.stem = cellData.therapeutic_genes.stem || 0;
    }

    // Map therapeutic types to legacy visualization types for color/shape
    const typeMapping = {
      biosensor: "V", // Blue - sensing
      effector: "D", // Red - treatment
      controller: "J", // Green - coordination
      stem: "S", // Purple - adaptation
      balanced: "balanced",
    };

    const mappedPrimary = typeMapping[primary] || "balanced";

    // Count actual gene types if we have gene array
    const geneTypes = { V: 0, D: 0, J: 0, Q: 0, S: 0 };

    if (Array.isArray(cellData.genes)) {
      // Count actual gene types from the array
      cellData.genes.forEach((gene) => {
        if (
          gene.is_active &&
          gene.gene_type &&
          geneTypes.hasOwnProperty(gene.gene_type)
        ) {
          geneTypes[gene.gene_type]++;
        }
      });
    } else {
      // Legacy: just assign all genes to the mapped primary type
      if (mappedPrimary in geneTypes) {
        geneTypes[mappedPrimary] = totalGenes;
      }
    }

    // Calculate diversity based on therapeutic gene types
    const activeTherTypes = Object.values(therapeuticGeneTypes).filter(
      (c) => c > 0
    ).length;
    const diversity = activeTherTypes / 4;
    const complexity = totalGenes / 10;

    // No quantum genes in therapeutic system
    const quantumGenes = 0;
    ...
    return {
      ...
      complexity,
      quantum_ratio: 0,
      active_ratio: 1, // All therapeutic genes are active
      therapeuticGeneTypes, // Include for debugging
    };
  }

  getSpecializationHue(specialization) {
    // For therapeutic visualization, map to therapeutic cell types
    const hueMap = {
      // Original gene types (kept for compatibility)
      V: 0.58, // Blue - now maps to biosensor
      D: 0.0, // Red - now maps to effector
      J: 0.33, // Green - now maps to controller
      Q: 0.83, // Magenta/Purple - quantum genes
      S: 0.75, // Purple - now maps to stem
      balanced: 0.5, // Cyan - balanced cells

      // Therapeutic cell types (direct mapping)
      biosensor: 0.58, // Blue (sensing)
      effector: 0.0, // Red (treatment)
      controller: 0.33, // Green (coordination)
      stem: 0.75, // Purple (adaptation)
    };

    // Handle both old gene types and new therapeutic types
    const primary = specialization.primary || specialization;
    return hueMap[primary] || 0.5;
  }
    ...
    cellData.genes.forEach((gene, index) => {
      ...
      switch (gene.gene_type) {
        ...
        default: // Fallback for any other type
          geometry = new THREE.SphereGeometry(indicatorSize * 0.8);
          break;
      }
      ...
      const radius = cellSize * 1.05 + 0.02 + gene.position * 0.1;
  ...
  updatePopulationStructure_OLD_UNUSED(state) {
    ...
    if (populationSize !== currentCellCount) {
      ...
      for (let i = 0; i < populationSize; i++) {
        ...
        const cellScale = Math.max(0.5, 1 - populationSize / 1000);
  ...
  animate() {
    ...
    this.cells.forEach((cell, cellId) => {
      // Make sure cellId exists and is a string
      if (!cellId || typeof cellId !== "string") {
        cellId = cell.userData?.id || "default";
      }
      ...
      if (specialization) {
        ...
        if (
          specialization.primary === "V" ||
          specialization.primary === "biosensor"
        ) {
          // V/biosensor cells are more active, darting movements
          const charCode = cellId.charCodeAt ? cellId.charCodeAt(0) : 0;
          cell.position.x += Math.sin(this.time * 2 + charCode) * 0.1;
          cell.position.z += Math.cos(this.time * 2 + charCode) * 0.1;
        } else if (
          specialization.primary === "D" ||
          specialization.primary === "effector"
        ) {
          // D/effector cells rotate more
          cell.rotation.y += 0.01 * specialization.diversity;
        } else if (
          specialization.primary === "J" ||
          specialization.primary === "controller"
        ) {
          // J/controller cells bob up and down
          const charCode = cellId.charCodeAt ? cellId.charCodeAt(0) : 0;
          cell.position.y += Math.sin(this.time * 3 + charCode) * 0.05;
        } else if (specialization.primary === "Q") {
          // Quantum cells phase in and out
          if (cell.userData.hub) {
            const charCode = cellId.charCodeAt ? cellId.charCodeAt(0) : 0;
            const phase = Math.sin(this.time * 4 + charCode);
      ...
      const floatCharCode =
        cellId && cellId.charCodeAt ? cellId.charCodeAt(0) : 0;
      cell.position.y += Math.sin(this.time + floatCharCode) * 0.02;
    });
