...
class LiveNeuralClockwork {
    ...
    if (state.cells && state.cells.length > 0) {
      ...
      state.cells.forEach((cellData, index) => {
        ...
        if (index === 0) {
          ...
          console.log("Is cellData.genes an array?", Array.isArray(cellData.genes));
  ...
  updateZoneCounts(zoneCounts) {
    ...
    if (memoryZoneEl)
      memoryZoneEl.textContent = `(${zoneCounts.memoryEliteZone})`;
    if (quantumZoneEl)
      quantumZoneEl.textContent = `(${zoneCounts.quantumDisk})`;
  ...
  determineCellSpecialization(cellData) {
    ...
    if (cellData.therapeutic_genes) {
      therapeuticGeneTypes.biosensor = cellData.therapeutic_genes.biosensor || 0;
      therapeuticGeneTypes.effector = cellData.therapeutic_genes.effector || 0;
      therapeuticGeneTypes.controller = cellData.therapeutic_genes.controller || 0;
    ...
    if (Array.isArray(cellData.genes)) {
      ...
      cellData.genes.forEach(gene => {
        if (gene.is_active && gene.gene_type && geneTypes.hasOwnProperty(gene.gene_type)) {
    ...
    const activeTherTypes = Object.values(therapeuticGeneTypes).filter(c => c > 0).length;
  ...
  animate() {
    ...
    this.cells.forEach((cell, cellId) => {
      ...
      if (specialization) {
        ...
        if (specialization.primary === "V" || specialization.primary === "biosensor") {
          // V/biosensor cells are more active, darting movements
          const charCode = cellId.charCodeAt ? cellId.charCodeAt(0) : 0;
          cell.position.x +=
            Math.sin(this.time * 2 + charCode) * 0.1;
          cell.position.z +=
            Math.cos(this.time * 2 + charCode) * 0.1;
        } else if (specialization.primary === "D" || specialization.primary === "effector") {
          // D/effector cells rotate more
          cell.rotation.y += 0.01 * specialization.diversity;
        } else if (specialization.primary === "J" || specialization.primary === "controller") {
          // J/controller cells bob up and down
          const charCode = cellId.charCodeAt ? cellId.charCodeAt(0) : 0;
          cell.position.y +=
            Math.sin(this.time * 3 + charCode) * 0.05;
      ...
      const floatCharCode = (cellId && cellId.charCodeAt) ? cellId.charCodeAt(0) : 0;
