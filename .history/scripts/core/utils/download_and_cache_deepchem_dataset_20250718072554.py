...
def download_and_cache_deepchem_datasets(datasets_to_prepare: list = None):
    ...
    for dataset_name in datasets_to_prepare:
        ...
        try:
            ...
            # Use CircularFingerprint which is available in this DeepChem version
            featurizer = dc.feat.CircularFingerprint(size=2048, radius=2)
            splitter = 'scaffold'
            
            try:
                logger.info(f"   Using CircularFingerprint featurizer")
                print(f"   Using CircularFingerprint featurizer")
