...
def _write_single_cell_architecture_state(cell_id, architecture_modifier):
    ...
    architecture_path = os.path.join("scripts", "visualization_data")
    ...
    with state_lock:
        ...
        # Also write to standard filename for visualization polling
        # Write to scripts/visualization_data which is where the visualization looks
        # __file__ is already in scripts/core/utils/, so go up 2 levels to get to scripts/
        architecture_json_path = os.path.join(architecture_path, 'architecture_state.json')
