...
def download_and_cache_deepchem_datasets(datasets_to_prepare: list = None, data_dir: str = None):
    ...
    for dataset_name in datasets_to_prepare:
        ...
        try:
            ...
            # Try different featurizer approaches - ONLY use ones that actually exist
            featurizer_attempts = []
            
            # Try MorganFingerprint first (this should exist)
            ...
            # Try CircularFingerprint as fallback
            ...
            # Try RDKitDescriptors as another option
            if hasattr(dc.feat, 'RDKitDescriptors'):
                try:
                    featurizer_attempts.append(('RDKitDescriptors', dc.feat.RDKitDescriptors()))
                except Exception as e:
                    logger.debug(f"RDKitDescriptors failed to instantiate: {e}")
