...
class SelfModifyingArchitecture(nn.Module):
    ...
    def _generate_layer_addition_params(self, state: torch.Tensor) -> Dict:
        ...
        mod_net = self.mod_networks['layer_addition']
        device = next(mod_net.parameters()).device
        state = state.to(device)
        param_vector = mod_net(state.flatten())
    ...
    def _generate_layer_removal_params(self) -> Dict:
        ...
        removal_probs = {}
        mod_net = self.mod_networks['layer_removal']
        device = next(mod_net.parameters()).device
        
        for name, module in self.dynamic_modules.items():
            if name != 'output' and len(self.dynamic_modules) > 2:
                # Don't remove output or if too few layers
                state_summary = torch.randn(self.base_dim).to(device)
                prob = mod_net(state_summary)
    ...
    def _generate_resizing_params(self, state: torch.Tensor) -> Dict:
        ...
        mod_net = self.mod_networks['dimension_resizing']
        device = next(mod_net.parameters()).device
        state = state.to(device)
        resize_params = mod_net(state.flatten())
    ...
    def _generate_activation_params(self, state: torch.Tensor) -> Dict:
        ...
        mod_net = self.mod_networks['activation_change']
        device = next(mod_net.parameters()).device
        state = state.to(device)
        activation_logits = mod_net(state.flatten())
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Ensure input is on the same device as the first dynamic module
        first_module = next(iter(self.dynamic_modules.values()))
