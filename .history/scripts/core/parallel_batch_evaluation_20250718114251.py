...
class OptimizedBatchEvaluator:
    ...
    def evaluate_population_batch(self, population: Dict, antigens: List[Data]) -> Dict[str, float]:
        ...
        MODIFIED: Now expects a list of antigen graphs already on the correct device.
        """
        # The 'antigens' list now contains graphs already moved to the target GPU.
        # We can directly create the batch object.
        try:
            antigen_batch = Batch.from_data_list(antigens)
        except RuntimeError as e:
            logger.error(f"Failed to create Batch object. This can happen if antigen graphs have inconsistent attributes. Error: {e}")
            # Return empty fitness scores if batching fails
            return {cid: 0.0 for cid in population.keys()}
                ...
                for batch_start in range(0, len(cells), batch_size):
                    ...
                    for cell in batch_cells:
                        ...
                        if batch_predictions:
                            ...
                            # Vectorized loss computation - ensure both tensors are on same device
                            losses = F.mse_loss(pred_stack, true_expanded.to(device), reduction='none').mean(dim=1)
