...
class ProductionGerminalCenter:
    ...
    def __init__(self, initial_population_size=None, device=None):
        ...
        self.hall_of_fame = {"champion": None, "fitness": -float('inf')}
        logger.info(f"Hall of Fame initialized with champion: {self.hall_of_fame['champion']} and fitness: {self.hall_of_fame['fitness']}")
        ...
        self.device = device or torch.device(cfg.device)
        # GPU optimization
        ...
        self.global_workspace = None # This will be set by the Archipelago
        self.intelligent_recombination_rate = cfg.intelligent_recombination_rate

        # *** NEW: Initialize the Hall of Fame ***
        # It will store the champion cell and its fitness.
    ...
    def _update_hall_of_fame(self, fitness_scores: Dict[str, float]):
        """
        Checks the current population for a new champion and updates the Hall of Fame.
        """
        if not self.population:
            return

        # Find the best cell in the current generation
        current_gen_champion_id = max(fitness_scores, key=fitness_scores.get)
        current_gen_champion_fitness = fitness_scores[current_gen_champion_id]

        # Compare with the all-time champion in the Hall of Fame
        if current_gen_champion_fitness > self.hall_of_fame["fitness"]:
            new_champion_cell = self.population[current_gen_champion_id]
            self.hall_of_fame["champion"] = new_champion_cell
            self.hall_of_fame["fitness"] = current_gen_champion_fitness
            island_info = f" on {self.island_desc}" if hasattr(self, 'island_desc') else ""
            logger.info(f"   🏆🏆🏆🏆 New Hall of Fame Champion{island_info}! Cell {new_champion_cell.cell_id[:8]} with fitness {current_gen_champion_fitness:.4f}🏆🏆🏆🏆")
        else:
            island_info = f" on {self.island_desc}" if hasattr(self, 'island_desc') else ""
            logger.info(f"   🏆 Current Hall of Fame Champion{island_info}: Cell {self.hall_of_fame['champion'].cell_id[:8]} with fitness {self.hall_of_fame['fitness']:.4f}")
...
# In the ProductionGerminalCenter class:
    ...
    def _selection_and_reproduction(self, fitness_scores: Dict[str, float]):
        ...
        Enhanced: Adds intelligent recombination for a subset of eliminated cells.
        """
        ...
        champion_cell = self.hall_of_fame.get("champion")
        champion_id = champion_cell.cell_id if champion_cell else None

        # Create a list of cell IDs to consider for selection, *excluding* the champion
        selectable_ids = [cid for cid in self.population.keys() if cid != champion_id]
        
        # Sort the selectable IDs by fitness
        sorted_selectable_ids = sorted(selectable_ids, key=lambda cid: fitness_scores.get(cid, 0), reverse=True)

        # Determine survivors and eliminated cells from the *selectable* population
        num_to_survive = int(len(sorted_selectable_ids) * (1 - self.selection_pressure))
        survivor_ids = sorted_selectable_ids[:num_to_survive]
        eliminated_ids = sorted_selectable_ids[num_to_survive:]
        
        # The parents for reproduction are the top selectable survivors
        parents = [self.population[cid] for cid in survivor_ids[:len(eliminated_ids)]]
        
        if champion_id:
            logger.info(f"   🛡️ Protecting Hall of Fame Champion {champion_id[:8]} from selection.")
        logger.info(f"   Selection complete: {len(survivor_ids)} survivors (+1 champion), {len(eliminated_ids)} to be replaced.")
        logger.info(f"   Selection complete: {len(survivor_ids)} survivors, {len(eliminated_ids)} to be recycled.")

        # Replace eliminated cells
        replaced_count = 0
        for i, cell_id_to_replace in enumerate(eliminated_ids):
            # *** NEW: Intelligent Recombination for Elites ***
            if i < len(parents) - 1 and random.random() < self.intelligent_recombination_rate:
                # *** ADDED LOGGING ***
                logger.info(f"   🧬 INTELLEGENT RECOMBINATION triggered for elite parents.")
                parent1 = parents[i]
                parent2 = parents[i+1]
                                
                # The breeder creates a new gene from the best parts of the parents
                # For simplicity, we'll recombine their first active genes
                ...
                if p1_gene and p2_gene:
                    new_gene = self.breeder.recombine(p1_gene, p2_gene)
                    child = ProductionBCell([new_gene]).to(cfg.device)
                else:
                    child = self.clone_pool.fast_clone(parents[i % len(parents)])
            else:
                # Standard fast cloning
        ...
        logger.info(f"   Recycled {replaced_count} cells as new offspring (with possible intelligent recombination).")
    ...
    def evolve_generation(self, antigens: List[Data], fitness_function=None):
        ...
        # Check if this is an island in an archipelago
        island_header = ""
        if hasattr(self, 'island_type'):
            island_header = f" [{self.island_emoji} {self.island_desc}]"
        
        logger.info(f"\n{'='*80}")
        logger.info(f"GENERATION {self.generation} on {self.device}{island_header}")
        logger.info(f"{'='*80}")
        ...
        # If no fitness function provided, use default (raw fitness)
        if fitness_function is None:
            def default_fitness(cell, results):
                return results.get('raw_fitness', 0.0)
            fitness_function = default_fitness
            
        fitness_scores = self.batch_evaluator.evaluate_population_batch(
            self.population, antigens, fitness_function
        )
        
        # Phase 1.5: Update Hall of Fame
        self._update_hall_of_fame(fitness_scores)
        
        # Clear CUDA cache after fitness evaluation (memory-intensive phase)
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
            logger.debug(f"   Cleared CUDA cache after fitness evaluation")
        ...
        # Clear CUDA cache after reproduction (cloning operations)
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
            logger.debug(f"   Cleared CUDA cache after reproduction phase")
        ...
        # Clear CUDA cache at the end of generation
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
            logger.debug(f"   Cleared CUDA cache at end of generation {self.generation}")
        ...
        from scripts.core.advanced.fitness_functions import print_cuda_memory_usage, clear_cuda_memory
        print_cuda_memory_usage()
        clear_cuda_memory()
        # --- End GPU Memory Debugging ---
    ...
    def reset(self):
        """
        Resets all stateful attributes of the germinal center to their initial values.
        This is crucial for running multiple independent experiments.
        """
        logger.info(f"   Resetting state for Germinal Center on {self.device}...")
        from scripts.core.production_germinal_center import ProductionBCell
        from scripts.core.parallel_batch_evaluation import OptimizedBatchEvaluator
        from scripts.core.clone_operation import FastClonePool
        from scripts.core.population_operations import VectorizedPopulationOps
        from scripts.core.phase_transition_detector import PhaseTransitionDetector
        from scripts.core.production_germinal_center import DreamConsolidationEngine
        ...
        self.transposition_events = []
        self.phase_transitions = []
        self.input_batch_history = deque(maxlen=10)
        self.mutation_log = deque(maxlen=500)
        self.plasmid_pool = deque(maxlen=200)
        ...
        # Re-create stateful components
        self.batch_evaluator = OptimizedBatchEvaluator(self.device)
        self.clone_pool = FastClonePool(device=self.device)
        self.vectorized_ops = VectorizedPopulationOps()
        self.phase_detector = PhaseTransitionDetector()
        self.dream_engine = DreamConsolidationEngine().to(self.device)
