...
class ProductionBCell(nn.Module):
    ...
    def __init__(self, initial_genes: List[ContinuousDepthGeneModule], device: Optional[torch.device] = None):
        super().__init__()
        self.cell_id = str(uuid.uuid4())
                # --- THE FIX: Ensure correct device placement ---
        self.device = device or torch.device(cfg.device)
        
        # Move genes to the correct device BEFORE adding them to ModuleList
        self.genes = nn.ModuleList([g.to(self.device) for g in initial_genes])
        
        self.fitness_history = deque(maxlen=100)
        self.lineage = []
        
        # Finally, move the entire cell module itself. This is redundant but safe.
        self.to(self.device)
        ...
        self.lineage = []
        # --- THE FIX: Ensure correct device placement ---
        # If a device is provided, use it. Otherwise, fall back to the config.
        # Move the entire module (including all genes in nn.ModuleList) to the correct device.
        self.to(self.device)
...
# In the ProductionBCell class:
    ...
    def extract_plasmid(self) -> Optional[Dict]:
        ...
        """
        # *** CRITICAL FIX: Determine the device from the cell ***
        device = next(self.parameters()).device
        
        high_fitness_genes = [g for g in self.genes if g.is_active and g.fitness_contribution > 0.7]
        ...
        for gene in selected_genes:
            try:
                ...
                # *** CRITICAL FIX: Ensure the cloned gene is on the same device ***
                plasmid_genes.append(new_gene.to(device))
...
# In scripts/core/production_b_cell.py -> class ProductionBCell

    def clone(self) -> 'ProductionBCell':
        """
        Create offspring with mutations and epigenetic inheritance.
        This version is optimized and hardened to ensure all created tensors
        and modules are placed on the correct GPU device, preventing cross-device errors.
        """
        # 1. Determine the correct device from the parent cell itself.
        #    All new objects must be moved to this device.
        device = next(self.parameters()).device
        ...
        for gene in self.genes:
            if gene.is_active:
                ...
                # 2. Create the new gene. Prioritize the gene's own specialized
                #    clone method if it exists, as it's the most reliable.
                try:
                    if hasattr(gene, 'clone'):
                        new_gene = gene.clone()
                    else:
                        # Fallback to a standard deepcopy if no custom clone method is found.
                        new_gene = copy.deepcopy(gene)
                except Exception as e:
                    logger.error(f"Failed to clone gene {gene.gene_id}: {e}. Skipping this gene.")
                    continue

                # 3. CRITICAL FIX: Immediately move the newly created gene to the
                #    parent's device. This is the key step to prevent device mismatches.
                new_gene = new_gene.to(device)
                
                # 4. Handle spontaneous transposition. The transpose method may return
                #    an additional new gene (e.g., from duplication).
                if random.random() < 0.05:
                    # The transpose method should also be device-aware, but we ensure it here.
                    transposed_child, _ = new_gene.transpose(0.1, 0.5)
                    if transposed_child:
                        child_genes.append(transposed_child.to(device))
                
                child_genes.append(new_gene)
        
        # 5. Create the new child cell with its list of genes, ensuring the cell
        #    itself is also on the correct device.
        child = ProductionBCell(child_genes).to(device)
        
        # 6. Inherit attributes and apply mutations.
        child.lineage = self.lineage + [self.cell_id]
        
        # Inherit regulatory matrix with noise, ensuring the new tensor is on the correct device.
        with torch.no_grad():
        ...
        # Apply mutations to the new child.
        child._mutate()
