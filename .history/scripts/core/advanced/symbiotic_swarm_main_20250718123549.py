...
def run_symbiotic_swarm_test(
    ...
    try:
        ...
        for generation in range(generations):
            ...
            if torch.cuda.is_available():
                ...
                logger.debug(f"Cleared CUDA cache after generation {generation + 1}")

    finally:
        # *** NEW: Ensure the thread pool is shut down ***
    ...
    for i, island_gc in enumerate(archipelago.islands):
        ...
        print(f"  Final Mean Fitness: {final_fitness}")
        print(f"  Final Diversity (Shannon): {final_diversity}")
