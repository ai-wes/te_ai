...
class ArchipelagoGerminalCenter:
    ...
    def __init__(self, population_per_island: int = 64):
        ...
        for i, device_str in enumerate(self.island_devices):
            ...
            # Create the GC and immediately move its entire model structure to the target device
            gc = ProductionGerminalCenter(initial_population_size=population_per_island)
            gc.to(device)
    ...
    def _evolve_island_worker(self, island_gc, antigen_batch):
        ...
        # Get the target device for this specific island
        island_device = next(island_gc.parameters()).device
    ...
    def _migrate(self):
        ...
        for i, island_gc in enumerate(self.islands):
            ...
            logger.info(f"   Island {i+1} (on {self.island_devices[i]}) is sending its top {len(sorted_cells[:num_migrants])} cells.")
        ...
        for i, target_island_gc in enumerate(self.islands):
            ...
            # Add the new, high-fitness migrants
