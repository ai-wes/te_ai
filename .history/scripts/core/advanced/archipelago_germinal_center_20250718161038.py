...
class ArchipelagoGerminalCenter:
    ...
    def evolve_generation(self, antigen_batch):
        ...
        # --- NEW: Run the Breeding Summit every 5 generations ---
        if self.generation % 5 == 0 and self.generation > 0:
    ...
    def _run_breeding_summit(self):
        ...
        for i, island_gc in enumerate(self.islands):
            ...
            logger.info(f"   Island {i+1} Champion: Cell {champion.cell_id[:8]} (Fitness: {champion.fitness_history[-1]:.4f}) on {island_gc.device}")
