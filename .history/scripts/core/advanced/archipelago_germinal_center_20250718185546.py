    ...
    def _run_breeding_summit(self):
        ...
        for i, island_gc in enumerate(self.islands):
            ...
            if champion:
                ...
                logger.info(f"   {island_gc.island_emoji} {island_gc.island_desc} Champion: Cell {champion.cell_id[:8]} (Fitness: {fitness:.4f}) on {island_gc.device}")
            else:
                logger.warning(f"   {island_gc.island_emoji} {island_gc.island_desc} has no champion in its Hall of Fame yet.")
        ...
        for i in range(self.num_islands):
            ...
            source1_gc = self.islands[i]
            source2_gc = self.islands[(i + 1) % self.num_islands]
            logger.info(f"\n   Breeding Champions from {source1_gc.island_emoji} {source1_gc.island_desc} and {source2_gc.island_emoji} {source2_gc.island_desc}...")
            ...
            if p1_gene and p2_gene:
                ...
                if target_island_gc.population:
                    ...
                    logger.info(f"   Injecting Perfect Spawn {perfect_spawn_cell.cell_id[:8]} into {target_island_gc.island_emoji} {target_island_gc.island_desc} (on {target_device}).")
