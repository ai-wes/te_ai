...
from scripts.core.causal_tapestry import CausalTapestry # New import
logger = get_logger()
from scripts.core.utils.benchmark_evaluator import benchmark_champion # <-- NEW IMPORT


ARCHIPELAGO = ArchipelagoGerminalCenter(
    population_per_island=cfg.population_per_island
)
CAUSAL_TAPESTRY = CausalTapestry() # New instance


def run_symbiotic_swarm_test(run_id: str):
    """
    ...
    logger.info(f"{'='*60}\n")
    CAUSAL_TAPESTRY.reset(run_id) # New instance
    ...
    # --- NEW: BENCHMARKING PHASE ---
    
    # Reload the test set (it was created during the initial data load)
    _, _, test_antigens = converter.convert_molnet_dataset(cfg.dataset_name)
    
    print("\n" + "="*60)
    print("🏆 FINAL BENCHMARK REPORT ON UNSEEN TEST DATA 🏆")
    print("="*60)
    ...
    for i, island_gc in enumerate(archipelago.islands):
        ...
        if champion:
            print(f"\n--- Benchmarking Champion from '{specialization.upper()}' Island ---")
            
            # Run the benchmark
            metrics = benchmark_champion(champion, test_antigens, device=island_gc.device)
            ...
            if metrics:
                ...
                print("  - Benchmarking failed for this champion.")
        else:
            print(f"\n--- No champion found for '{specialization.upper()}' Island to benchmark ---")
...
if __name__ == "__main__":
    ...
    run_symbiotic_swarm_test(run_id="drug_discovery_run_1")
    ARCHIPELAGO.causal_tapestry.save_tapestry("tapestry_run_1.graphml")
