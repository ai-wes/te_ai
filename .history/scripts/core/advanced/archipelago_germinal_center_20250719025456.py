...
from scripts.core.advanced.fitness_functions import CHAMPION_ARCHIVE # <-- NEW IMPORT
...
# In scripts/core/archipelago_germinal_center.py
    ...
    def _run_breeding_summit(self):
        ...
        for i, island_gc in enumerate(self.islands):
            ...
            # --- NEW: Update the global champion archive ---
            if champion:
                active_genes = [g for g in champion.genes if g.is_active]
                fingerprint = "-".join(sorted([g.gene_type for g in active_genes]))
                if fingerprint not in CHAMPION_ARCHIVE:
                    CHAMPION_ARCHIVE.append(fingerprint)
                    logger.info(f"   New champion added to archive: {fingerprint}")
                else:
                    logger.info(f"   Champion already in archive: {fingerprint}")
            
            # --- NEW: Log the archive size ---
            logger.info(f"   Current champion archive size: {len(CHAMPION_ARCHIVE)}")
        ...
        # --- Causal Tapestry Logging: Add event node and log champion participation ---
        event_id = f"summit_g{self.generation}"
        self.causal_tapestry.add_event_node(event_id, "BREEDING_SUMMIT", self.generation, {"num_islands": self.num_islands})
        for champion in champions:
            if champion:
                self.causal_tapestry.log_event_participation(champion.cell_id, event_id, role="champion")
        ...
        for i in range(self.num_islands):
            ...
            if p1_gene and p2_gene:
                ...
                # FINAL GUARANTEE: Move the cell to the correct device
                perfect_spawn_cell = perfect_spawn_cell.to(target_device)

                # --- Causal Tapestry Logging: Add cell node, log lineage, log event output ---
                self.causal_tapestry.add_cell_node(perfect_spawn_cell.cell_id, self.generation, target_island_gc.island_name, -1.0, [g.gene_id for g in perfect_spawn_cell.genes])
                self.causal_tapestry.log_lineage(parent1.cell_id, perfect_spawn_cell.cell_id)
                self.causal_tapestry.log_lineage(parent2.cell_id, perfect_spawn_cell.cell_id)
                self.causal_tapestry.log_event_output(event_id, perfect_spawn_cell.cell_id, role="perfect_spawn")
