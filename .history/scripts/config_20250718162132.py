...
class CFG:
    ...
    generations: int = 30
    num_islands: int = 3
    population_per_island: int = 64
    island_devices: List[str] = field(default_factory=lambda: ["cuda:0", "cuda:0", "cuda:0"])
    num_migrants: int = 5  # Number of individuals to migrate between islands
    migration_frequency: int = 5  # How often migrations happen (every N generations)
    breeding_summit_frequency: int = 5  # The number of generations between each Archipelago Breeding Summit.
    migration_frequency: int = 10  # How often migrations happen (every N generations)
