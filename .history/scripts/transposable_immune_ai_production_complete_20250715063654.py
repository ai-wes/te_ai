        ...
        with torch.no_grad():
            ...
            for gene in cell.genes:
                if gene.is_active:
                    ...
                    # This makes the code robust even if some genes are on different devices.
                    try:
                    ...
                    if quantum_outcome == 0:
                        ...
                        for param in gene.parameters():
                            # Ensure all tensors are on the same device as the parameter
                            noise = torch.randn_like(param)  # This will be on the same device as param
                        ...
                        for param in gene.parameters():
                            # Ensure all tensors are on the same device as the parameter
                            noise = torch.randn_like(param)  # This will be on the same device as param
                            param.data += learning_rate * 2.0 * noise * affinity_on_device.to(param.device)
...
class ProductionBCell(nn.Module):
    ...
    def _compute_gene_regulation(self, active_genes: List) -> torch.Tensor:
        ...
        # Extract regulatory submatrix - ensure it matches n_genes
        matrix_size = min(n_genes, self.gene_regulatory_matrix.size(0))
        reg_matrix = self.gene_regulatory_matrix[:matrix_size, :matrix_size]
        
        # If we have more genes than matrix size, pad with identity
        if n_genes > matrix_size:
            device = self.gene_regulatory_matrix.device
            padded_matrix = torch.eye(n_genes, device=device) * 0.1
            padded_matrix[:matrix_size, :matrix_size] = reg_matrix
            reg_matrix = padded_matrix
...
# In the ProductionGerminalCenter class:
    ...
    def _detect_population_stress(self, metrics: Dict[str, float]) -> float:
        ...
        stress_factors = []
        # Factor 1: Fitness stagnation (This now becomes the primary driver)
        if len(self.fitness_landscape) >= cfg.stress_window:
            ...
            # Check for a statistically significant downward trend
            slope, intercept, r_value, p_value, std_err = stats.linregress(range(len(recent_fitness)), recent_fitness)
            
            # Only consider the trend stressful if it's negative AND statistically significant (p < 0.05)
