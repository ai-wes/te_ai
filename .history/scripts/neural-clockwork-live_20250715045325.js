// neural-clockwork-live.js - Crystalline Neural Organism Visualization
...
class TransposableNeuralCell {
  constructor() {
    ...
    this.zones = {
      memoryEliteZone: {
        center: new THREE.Vector3(0, 100, 0), // Top of central cylinder
        radius: 30,
        height: 40,
        color: 0x44aaaa, // Cyan
      ...
      lightZone: {
        // Selection zone - helix pattern around central cylinder
        center: new THREE.Vector3(0, 0, 0),
        radius: 60, // Radius from center
        height: 120, // Height of helix
        color: 0xaaaa44, // Yellow-green
        description: "Selection Light Zone - helix pattern",
      },
      darkZone: {
  ...
  memoryEliteZonePosition(cellData, index) {
    // Elite cells cluster at the top of the central cylinder
    const zone = this.zones.memoryEliteZone;
    const fitness = cellData.fitness || 0.5;

    // Higher fitness cells get closer to the very top
    const heightOffset = (1 - fitness) * zone.height;
    const angle = (index / 10) * Math.PI * 2; // Spiral arrangement
    const radiusScale = 0.3 + fitness * 0.7; // Fitness determines distance from center

    const x = Math.cos(angle) * zone.radius * radiusScale;
    const z = Math.sin(angle) * zone.radius * radiusScale;
    const y = zone.center.y - heightOffset;
  ...
  lightZonePosition(cellData, index, population) {
    ...
    const t = (index / totalCells) * 4; // 4 full turns
    const angle = t * Math.PI * 2;

    // Helix parameters
    const helixRadius = zone.radius * (0.8 + fitness * 0.2); // Fitter cells spiral closer
    const helixHeight = zone.height * (t / 4); // Vertical progression
  ...
  createMemoryEliteZone(scene) {
    ...
    // Add crown effect at top
    const crownGeometry = new THREE.TorusGeometry(zone.radius * 1.2, 5, 8, 16);
    ...
    crown.position.set(
      zone.center.x,
      zone.center.y + zone.height / 2,
      zone.center.z
    );
    crown.rotation.x = Math.PI / 2;
    this.zoneGroup.add(crown);
  }
  ...
  createQuantumDisks(scene) {
    ...
    // Top disk
    const topDiskGeometry = new THREE.CylinderGeometry(
      zone.radius,
      zone.radius,
      zone.thickness,
      64
    );
    const topDiskMaterial = new THREE.MeshPhysicalMaterial({
      color: zone.color,
      emissive: zone.color,
    ...
    const topDisk = new THREE.Mesh(topDiskGeometry, topDiskMaterial);
    topDisk.position.copy(zone.topCenter);
    topDisk.frustumCulled = false;
    this.zoneGroup.add(topDisk);

    // Bottom disk
    const bottomDisk = new THREE.Mesh(topDiskGeometry, topDiskMaterial);
    bottomDisk.position.copy(zone.bottomCenter);
    bottomDisk.frustumCulled = false;
    this.zoneGroup.add(bottomDisk);
  }
